...
同梱される Codex、GUI、その他のバイナリは配布対象にしない。
README の説明用 SVG はソースに含めるが、配布アーカイブには含めない。
...
README のテキスト操作例を更新したら、リポジトリのルートで画像を再生成する。

```sh
ruby scripts/render-terminal-demo.rb > docs/assets/terminal-demo.svg
```

生成処理は README を読み取るだけで、CLI、Keychain、公式アプリを操作しない。
操作例には架空のアカウントを使い、実装の表示に合わせる。

## 公開前の確認

`scripts/check.sh` が、Swift テスト、Release CLI ビルド、全配布スクリプトの構文確認を
行い、README の操作例と SVG の一致を確認した後、
`scripts/check-distribution.sh` と `scripts/check-release.sh` を呼び出す。
...
代入、個人のメールアドレス、個人の絶対パス、非テキストのバイナリを検出する。
SVG も XML テキストとして同じ内容検査を受ける。公開対象のファイルに合成値用の例外や
マーカーはなく、テスト用の値は実行時に隔離した一時
