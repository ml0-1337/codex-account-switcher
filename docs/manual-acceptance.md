...
消えることがある。また、アプリ内のタスク(スレッド)表示がバックエンド再起動後に読み込め
なくなることが実測で確認されている。エージェントのセッション自体は新バックエンドへ再接続
して継続する場合があるが、表示を回復するにはアプリの再起動が必要になることがある。
再起動前に、切り替えの終了ステータス、成功表示、auth.json・config.toml の比較結果、
